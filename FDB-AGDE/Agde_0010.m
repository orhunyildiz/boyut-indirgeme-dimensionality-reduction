function [bestSolution, bestFitness, iter]=Agde_0010(train, test, maxIteration, k)
    ...
    for j = 1:NP % for each individual
         %%%%%%%%ADAPTIVE CR RULE  %%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %%%%%%%%%%%%%%%%%END OF CR RULE%%%%%%%%%%%%%%%%%%%%%%%%%%%
                f=Fit;
                if rand() < w
                    in = fitnessDistanceBalanceIndexes( Pop', Fit);
                else
           ...
           paraIndex=floor (rand(1,1)*length(AA))+1;
                ...
                F=0.1+0.9*rand(1,1);
                rouletFdbIndex = rouletteFitnessDistanceBalance( Pop', Fit);
                Rnd = floor(rand()*D) + 1;
                for i = 1:D
                    if ( rand()<CR ) || ( Rnd==i )
